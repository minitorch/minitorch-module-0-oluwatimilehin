...
def test_sigmoid(a: float, b: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert sigmoid(max(a, b)) >= sigmoid(
        min(a, b)
    )  # technically not strictly increasing with the "equal" option but oh well

    assert val >= 0
    assert val <= 1
    assert round(1.0 - val, 5) == round(sigmoid(-a), 5)
...
def test_distribute(a: float, b: float, c: float) -> None:
    ...
    """
    assert round(mul(c, add(a, b)), 4) == round(add(mul(c, a), mul(c, b)), 4)
...
def test_other(a: float, b: float) -> None:
    ...
    if is_close(a, b):
        assert round(sigmoid(a), 2) == round(sigmoid(b), 2)
